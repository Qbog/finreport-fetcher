...
def test_index_raw_store_uses_raw_provider_without_redundant_index_folder(tmp_path: Path):
    store = RawIndexStore(tmp_path / "_global" / "indexes" / "sh000001")
    df = pd.DataFrame([
    ...
    assert (tmp_path / "_global" / "indexes" / "sh000001" / "raw" / "tencent" / "current" / "daily.pkl").exists()
    assert not (tmp_path / "_global" / "indexes" / "sh000001" / "raw" / "index" / "tencent" / "current" / "daily.pkl").exists()
