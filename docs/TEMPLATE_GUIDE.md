...
- 当你运行：`finchart run --start ... --end ...` 且模板 `mode = "compare"` 时：
  - 会对**时间范围内每个报告期**分别生成 1 张比较图（也会生成同名的 Excel）。
- 如果你希望只生成“某一个期末”的比较图：
  - 运行时传 `--as-of 2024-12-31`，或在模板里写 `period_end = "2024-12-31"`。

### `[[bars]]` 颜色与嵌套（分组）

每个 `[[bars]]` 叶子节点都可以指定颜色（十六进制）：

```toml
[[bars]]
name = "货币资金"
expr = "bs.cash"
color = "#FFFFC0"   # 也支持中文键：颜色 = "#FFFFC0"
```

支持嵌套分组（分组节点不需要 `expr`，只用于组织与继承）：
...
color = "#4E79A7"
  ...
  [[bars.children]]
  name = "货币资金"
  expr = "bs.cash"       # 若不写 color，会继承上面的分组颜色
...
color = "#E15759"
  ...
  [[bars.children]]
  name = "应付账款"
  expr = "bs.accounts_payable"
```

说明：
- 分组的 `statement/color` 会向下继承，子项可覆盖。
- compare 模式会按展开后的顺序输出柱子；目前分组节点本身不绘制柱子。
