...
def merge(
    ...
    templates: str = typer.Option("templates", "--templates", help="模板目录（单模板单文件 *.toml）"),
    ...
    category_config: str | None = typer.Option(None, "--category-config", help="分类配置文件路径（默认：config/company_categories.toml）"),
    start: str = typer.Option(..., "--start"),
    end: str = typer.Option(..., "--end"),
    data_dir: str | None = typer.Option(None, "--data-dir", help="财报数据目录（默认：若 ./output 存在则用 output，否则用当前目录）"),
    out_dir: str | None = typer.Option(None, "--out", help="输出目录（默认：每家公司输出到 {data_dir}/{公司名}_{code6}/charts/）"),
